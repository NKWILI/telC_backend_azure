...
- [x] **T2** Password rule → **one rule everywhere, matching the landing page: at least 8 characters, at least 1 uppercase letter, 1 digit and 1 special character, at most 72 UTF-8 bytes.** Applies to centers and students (landing page, center dashboard, Flutter app, backend) whenever a password is **set**: register, reset, change password, and a student setting a password. **Never checked at login**, so existing accounts with older passwords keep working. Error: `400 VALIDATION_ERROR` (field `password`). **Backend: done for centers and students (students 2026-09-19: register and reset-password).** The landing page register asks ≥6 + complexity and settings ≥8 — both move to this rule (frontend prompts).
